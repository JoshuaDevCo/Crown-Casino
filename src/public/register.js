...
function Register() {
  ...
  function onClickSubmit(e) {
    ...
    doRegister(nickName, email, password, birthDate)
      .then(response => {
        if (response) {
          navigate('/login')
        }
  ...
  return (
    <main className="flex bg-slate-800 h-screen w-screen items-center justify-center ">
        ...
        <form className="content-center">
          <Link to='/' className="flex items-center justify-center font-semibold">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5">
              <path fill-rule="evenodd" d="M17 10a.75.75 0 01-.75.75H5.612l4.158 3.96a.75.75 0 11-1.04 1.08l-5.5-5.25a.75.75 0 010-1.08l5.5-5.25a.75.75 0 111.04 1.08L5.612 9.25H16.25A.75.75 0 0117 10z" clipRule="evenodd" />
            </svg> Back to HomePage</Link>
          <img src={GG} alt="logo" className="w-14 h-14 mx-auto my-2 bg-black rounded-full p-2" />
          <h1 className="text-center m-5 font-bold text-grey-800 text-lg">Singup Now!</h1>
          <div className="form-group mb-6">
            <label htmlFor="labelNickname" className="form-label inline-block mb-2 text-gray-700 text-md font-semibold">Choose your Nickname<br></br></label>
            ...
            focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" id="nickname"
              placeholder="What is your nickname?" onChange={(e) => setNickName(e.target.value)} />
            ...
            <label htmlFor="floatingInput" className="form-label inline-block mt-2 mb-2 text-gray-700 text-md font-semibold">Borning date</label>
            <input type="date"
              className="datepicker form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
              placeholder="Select a date" onChange={(e) => setBirthDate(e.target.value)} />
            <input type="checkbox"
              ...
              id="exampleCheck1" />
            <label className="form-check-label inline-block text-gray-800 text-sm mt-3" htmlFor="exampleCheck1">I agree with terms and conditions</label>
      ...
      transition
      duration-150
      ease-in-out" onClick={(e) => onClickSubmit(e)}>Submit</button>
          </div>
          {
            error ?
